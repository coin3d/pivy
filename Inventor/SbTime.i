...
def __init__(self,*args):
   ...
   if len(args) == 1:
      if type(args[0]) == type(1.0):
         newobj = apply(_coin.new_SbTime_d,args)
      else:
         newobj = apply(_coin.new_SbTime_tv,args)
   elif len(args) == 2:
      newobj = apply(_coin.new_SbTime_i_l,args)
   else:
      newobj = apply(_coin.new_SbTime,args)
   if newobj:
...
def setValue(*args):
   if len(args) == 2:
      if type(args[1]) == type(1.0):
         return apply(_coin.SbTime_setValue,args)
      elif type(args[1]) == SbTime:
         return _coin.SbTime_setValue(args[0],args[1].getValue())
      else:
         return apply(_coin.SbTime_setValue_tv,args)
   elif len(args) == 3:
      return apply(_coin.SbTime_setValue_i_l,args)   
   return apply(_coin.SbTime_setValue,args)
%}
