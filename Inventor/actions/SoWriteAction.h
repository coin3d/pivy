...
def __init__(self,*args):
   if len(args) == 1:
      self.this = apply(pivyc.new_SoWriteAction_out,args)
      self.thisown = 1
      return
   self.this = apply(pivyc.new_SoWriteAction,args)
   self.thisown = 1
%}
...
def continueToApply(*args):
   if isinstance(args[1], SoNode):
      return apply(pivyc.SoWriteAction_continueToApply_nod,args)
   return apply(pivyc.SoWriteAction_continueToApply,args)
%}
