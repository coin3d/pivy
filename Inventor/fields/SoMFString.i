...
def setValues(*args):
   if isinstance(args[3], SbString):
      return apply(_coin.SoMFString_setValues_i_i_str,args)
   return apply(_coin.SoMFString_setValues,args)
%}
...
def setValues(*args):
   if len(args) == 2:
      if isinstance(args[1], SoMFString):
         ...
         return _coin.SoMFString_setValues(args[0],0,len(val),val)
      else:
         return _coin.SoMFString_setValues(args[0],0,len(args[1]),args[1])
   elif len(args) == 3:
      if isinstance(args[2], SoMFString):
         val = args[2].getValues()
         return _coin.SoMFString_setValues(args[0],args[1],len(val),val)
      else:
...
%pythoncode %{
   def getValues(*args):
     if len(args) == 1:
        return _coin.SoMFString___getValuesHelper__(args[0], 0)
     return _coin.SoMFString___getValuesHelper__(*args)
%}
}
